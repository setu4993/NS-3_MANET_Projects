...
//  Config::SetDefault ("ns3::WifiRemoteStationManager::RtsCtsThreshold", UintegerValue (1)); // enable rts cts all the time.
  ...
  anim.UpdateNodeColor(servernodes.Get (1), 0, 0, 255);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
	  ...
	  if ((t.sourceAddress == "10.0.0.1") || (t.destinationAddress == "10.0.0.1") || (t.sourceAddress == "10.0.0.2") || (t.destinationAddress == "10.0.0.2"))
	  {
...
*/
  ...
  Ptr<ListPositionAllocator> positionAllocS1 = CreateObject<ListPositionAllocator> ();
  positionAllocS1->Add(Vector(565.0, 115.0, 0.0));
  mobility.SetPositionAllocator(positionAllocS1);
  mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
  mobility.Install (servernodes.Get (0));
  
  Ptr<ListPositionAllocator> positionAllocS2 = CreateObject<ListPositionAllocator> ();
  positionAllocS2->Add(Vector(635.0, 115.0, 0.0));
  mobility.SetPositionAllocator(positionAllocS2);
  mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
...
GpsrExample::InstallApplications ()
{
  ...
  ApplicationContainer apps1 = server1.Install (servernodes.Get(0));
  apps1.Start (Seconds (2));
  apps1.Stop (Seconds (totalTime));
  
  UdpEchoServerHelper server2 (port);
  //uint16_t server1Position = size-1; //bottom right
  ApplicationContainer apps2 = server2.Install (servernodes.Get(0));
  apps2.Start (Seconds (2));
  apps2.Stop (Seconds (totalTime));

	UdpEchoClientHelper client1 (servinterface.GetAddress (0), port);
	client1.SetAttribute ("MaxPackets", UintegerValue (maxPacketCount));
	client1.SetAttribute ("Interval", TimeValue (Seconds (timeInterval)));
  ...
  */
  ...
  for(uint16_t i = 0; i < size; i++)
  {
	...
	apps1 = client1.Install (nodes.Get (i));
	apps1.Start (Seconds (2.1 + (0.1 * i)));
	apps1.Stop (Seconds (totalTime));
	...
	UdpEchoClientHelper client2 (servinterface.GetAddress (0), port);
	client2.SetAttribute ("MaxPackets", UintegerValue (maxPacketCount));
	client2.SetAttribute ("Interval", TimeValue (Seconds (timeInterval)));
	...
	for(uint16_t i = 0; i < size; i++)
  {
	//uint16_t clientPosition = 0; //top left
	//client2.SetAttribute ("Interval", TimeValue (Seconds (timeInterval)));
	apps2 = client2.Install (nodes.Get (i));
	apps2.Start (Seconds (17.3 + (0.1 * i)));
	apps2.Stop (Seconds (totalTime));
