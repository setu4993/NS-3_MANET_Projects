...
GpsrExample::InstallApplications ()
{
  ...
  apps.Start (Seconds (5.0)); // Server Start Time
  ...
  uint16_t clientPostion = 4; // bottom-left
  apps = client.Install (nodes.Get (clientPostion));
  apps.Start (Seconds (6.0)); // Client Start Time
  apps.Stop (Seconds (totalTime-0.1)); // Client Stop Time

  // Move away nodes 0 and 6, in order to create test scenario
  uint16_t nodesToRemove[] = {0, 6};
  Ptr<Node> node;
